...
static void replace_all_inplace(std::string& text, const std::map<std::string, std::string>& replacements) {
...
BpeEncodeLayer::BpeEncodeLayer(std::string token_model_path, bool has_bos, bool has_eos)
  ...
  for (auto it = vocabs.begin(); it != vocabs.end(); ++it) {
    const auto cpts = unicode_cpts_from_utf8(it.key());
    std::string key;
    for (const auto cpt : cpts) {
      const auto utf8 = unicode_cpt_to_utf8(cpt);
      key += unicode_utf8_to_byte(utf8);
    }
    const int32_t id = it.value();
    encoder[key] = id;
  }
...
std::vector<int32_t> BpeEncodeLayer::encode(const std::string& sentence) const {
  ...
  replace_all_inplace(s, replacements);
...
std::string BpeEncodeLayer::decode(const std::vector<int32_t>& token_ids) const {
  ...
  replace_all_inplace(sentence, reverse_replacements);
  return sentence;
}
