...
def _read_safetensors_index(safetensors_path):
...
def _write_tensor_from_safetensors(out_file, mm, data_base_offset, tensor_info):
    dtype = tensor_info['dtype']
    start, end = tensor_info['data_offsets']
    raw = mm[data_base_offset + start:data_base_offset + end]

    if dtype == 'BF16':
        arr_u16 = np.frombuffer(raw, dtype=np.uint16)
        arr_u32 = arr_u16.astype(np.uint32) << 16
        arr_fp32 = arr_u32.view(np.float32)
        out_file.write(arr_fp32.tobytes())
    elif dtype == 'F32':
        out_file.write(raw)
    else:
...
def export_qwen2_safetensors_to_version1(model_path, filepath):
    config_path = os.path.join(model_path, 'config.json')
    safetensors_path = os.path.join(model_path, 'model.safetensors')
    if not os.path.exists(config_path) or not os.path.exists(safetensors_path):
        raise FileNotFoundError('config.json or model.safetensors not found')
    ...
    header_len, header = _read_safetensors_index(safetensors_path)
    data_base_offset = 8 + header_len

    with open(safetensors_path, 'rb') as sf, open(filepath, 'wb') as out_file:
        import mmap
        mm = mmap.mmap(sf.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            out_file.write(struct.pack('I', 0x616b3432))
            out_file.write(struct.pack('i', 1))
            out_file.write(struct.pack('iiiiiii', dim, hidden_dim, n_layers, n_heads,
            ...
            for key in ordered_keys:
                ...
                _write_tensor_from_safetensors(out_file, mm, data_base_offset, header[key])
        finally:
            mm.close()
...
def load_hf_model(model_path):
    ...
    safetensors_path = os.path.join(model_path, 'model.safetensors')
    if load_safetensors_torch is not None and os.path.exists(safetensors_path):
        if not hasattr(torch, 'frombuffer'):
            ...
            if hasattr(torch, 'bfloat16'):
                _dtype_map[torch.bfloat16] = _np.uint16

            def _frombuffer_compat(buffer, *, dtype=torch.uint8, count=-1, offset=0, requires_grad=False):
                if dtype == getattr(torch, 'bfloat16', None):
                    arr_u16 = _np.frombuffer(buffer, dtype=_np.uint16, count=count, offset=offset)
                    arr_u32 = arr_u16.astype(_np.uint32) << 16
                    arr = arr_u32.view(_np.float32)
        ...
        hf_dict = load_safetensors_torch(safetensors_path, device='cpu')
        layers = []
        for i in range(config.n_layers):
            layers.append(SimpleNamespace(
                attention_norm=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.input_layernorm.weight']),
                attention=SimpleNamespace(
                    wq=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.self_attn.q_proj.weight']),
                    wk=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.self_attn.k_proj.weight']),
                    wv=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.self_attn.v_proj.weight']),
                    wo=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.self_attn.o_proj.weight']),
                ),
                ffn_norm=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.post_attention_layernorm.weight']),
                feed_forward=SimpleNamespace(
                    w1=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.mlp.gate_proj.weight']),
                    w2=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.mlp.down_proj.weight']),
                    w3=SimpleNamespace(weight=hf_dict[f'model.layers.{i}.mlp.up_proj.weight']),
                ),
            ))

        output_weight = hf_dict.get('lm_head.weight', hf_dict['model.embed_tokens.weight'])
        model = SimpleNamespace(
            params=config,
            layers=layers,
            tok_embeddings=SimpleNamespace(weight=hf_dict['model.embed_tokens.weight']),
            norm=SimpleNamespace(weight=hf_dict['model.norm.weight']),
            output=SimpleNamespace(weight=output_weight),
        )
        return model
    ...
    try:
        ...
        print("Error: transformers or safetensors package is required to load huggingface models")
    ...
    for i in range(config.n_layers):
        ...
        layers.append(SimpleNamespace(
            attention_norm=SimpleNamespace(weight=hf_layer.input_layernorm.weight),
            attention=SimpleNamespace(
                wq=SimpleNamespace(weight=hf_layer.self_attn.q_proj.weight),
                wk=SimpleNamespace(weight=hf_layer.self_attn.k_proj.weight),
                wv=SimpleNamespace(weight=hf_layer.self_attn.v_proj.weight),
                wo=SimpleNamespace(weight=hf_layer.self_attn.o_proj.weight),
            ),
            ffn_norm=SimpleNamespace(weight=hf_layer.post_attention_layernorm.weight),
            feed_forward=SimpleNamespace(
                w1=SimpleNamespace(weight=hf_layer.mlp.gate_proj.weight),
                w2=SimpleNamespace(weight=hf_layer.mlp.down_proj.weight),
                w3=SimpleNamespace(weight=hf_layer.mlp.up_proj.weight),
            ),
        ))

    output_weight = getattr(hf_model, 'lm_head', None)
    output_weight = output_weight.weight if output_weight is not None else hf_model.model.embed_tokens.weight
    model = SimpleNamespace(
        params=config,
        layers=layers,
        tok_embeddings=SimpleNamespace(weight=hf_model.model.embed_tokens.weight),
        norm=SimpleNamespace(weight=hf_model.model.norm.weight),
        output=SimpleNamespace(weight=output_weight),
    )
    return model
...
if __name__ == "__main__":
    ...
    if args.checkpoint:
        ...
        safetensors_path = os.path.join(args.hf, 'model.safetensors')
        if args.version == 1 and os.path.exists(safetensors_path):
            export_qwen2_safetensors_to_version1(args.hf, args.filepath)
            raise SystemExit(0)
